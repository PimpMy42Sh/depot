...
/*   Updated: 2016/05/10 10:21:22 by mfamilar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		check_env(char **environ)
{
	int		i;
...
static char		*append_path(char *environ, char *av)
{
	char	**path_env;
	char	*ret;
...
void			parse_arguments(char **environ, char *line, int boolean)
{
	...
	if (i >= 0)
		av[0] = append_path(environ[i], av[0]);
	map_environ(environ);
	suspend_terminal();
