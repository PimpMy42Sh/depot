...
/*   Updated: 2016/09/23 14:41:00 by Marco            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void parse_etc_path_directories(t_env *env)
{
	char	**all_path;
	int		i;
	...
	i = 0;
	path_manager(env, 0);
	all_path = ft_strsplit(env->etc_path, ':');
	while (all_path[i])
	{
...
void				parse_path_directories(t_env *env)
{
	...
	i = 0;
	if (!env->etc_path)
		parse_etc_path_directories(env);
